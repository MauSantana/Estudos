...
//        Arquivo.gravaArquivoCsv(psicologoObj, "psicologos");
        ArquivoTxt.gravaArquivoTxt(psicologos, "psicologos");
    ...
    public String logoff(@PathVariable String login) {
        ...
        for (Usuario usuarioDoFor : usuarios) {
            if (usuarioDoFor.getUserName().equals(login)) {

                fraseFinal = String.format("Usuário %s saiu do sistema", usuarioDoFor.getNome());

            } else {

                fraseFinal = String.format("Usuário %s não autenticado", login);
            }
    ...
    public void fazerConsulta(@PathVariable String msg, @PathVariable String nome,@PathVariable String email) {
        ...
        SlackMessage.messagemMulherSegura(consulta);
